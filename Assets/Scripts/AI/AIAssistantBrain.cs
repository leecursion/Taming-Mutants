...
public class AIAssistantBrain : MonoBehaviour
{
    ...
    // 리본 색은 이차구조(나선/가닥/고리)를 뜻한다 — 원자 단계의 pLDDT 신뢰도 색과 전혀 다른
    // 의미인데 둘 다 "색깔 지도"로 뭉뚱그리면, 리본의 노란 가닥을 보고 "AI가 확신 없는 곳"으로
    // 읽게 된다. 각 단계에서 그 화면의 색이 무슨 뜻인지 그때그때 못박아 준다.
    public string ribbonLine = "지금 보이는 게 단백질 전체 모양이에요. 색은 줄기가 어떤 모양으로 접혔는지 " +
                               "알려줘요 — 자주색은 돌돌 말린 나선, 노란색은 납작한 가닥이에요. " +
    ...
    // 원자 색은 원소가 아니라 pLDDT(AI 예측 신뢰도)다 — Atom.prefab에 붙은 PLDDTColorizer가
    // b-factor 값을 파랑~주황으로 칠한다. "색이 다르면 다른 원소"라고 말하면 화면과 다른
    // 이야기가 되고, 하필 그 색이 이 게임의 핵심 개념(AI가 어디를 확신하지 못하는가)이라
    // 오해가 그대로 학습된다.
    public string aminoAcidLine = "여기가 가장 안쪽이에요. 이제 공 하나하나가 원자예요. " +
                                  "색은 AI가 이 부분 모양을 얼마나 확신하는지 보여줘요 — " +
    ...
    // 대화 세대. ResetConversation()이 부를 때마다 하나 늘어난다. 그 앞에 나간 요청의 응답은
    ...
    /// <summary>
    /// "이전"으로 지금 화면을 떠날 때 부른다. 하던 말과 음성을 즉시 끊고, 아직 도착하지 않은
    /// LLM 응답은 나중에 와도 무시되게 세대를 올린다.
    ///
    /// 대본 브리핑/힌트 진행도 이 화면(단계)의 것이므로 함께 되돌린다 — 되돌리지 않으면 다음
    /// 화면에서 "이미 다 쓴 힌트"나 "이미 해설한 단계"로 오판해 아무 말도 하지 않는다.
    /// </summary>
    ...
    public void AskAssistant(string question, string selection = null)
    {
        ...
        client.Ask(question, BuildContext(selection),
            ...
            onFailed: _ => { if (generation == _conversationGeneration) SpeakNow(requestFailedNotice); });
    }
