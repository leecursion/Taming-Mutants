    ...
    private static void FillKrasG12C(QuestDefinition quest)
    {
        ...
        quest.stages = new[]
        {
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "여기 보이는 세포들에서 이상한 신호가 나오고 있어. 안으로 들어가서 살펴보자.",
                    "세포 속으로 더 들어가면, 우리 몸의 설계도인 DNA가 나선 모양으로 보일 거야.",
                    "설계도 글자가 딱 하나 바뀌었어! 그래서 KRAS라는 단백질 모양이 살짝 달라졌지.",
                },
                hints = new[]
                {
                    "반짝이는 자리가 12번 자리야. 나선을 따라 천천히 위로 살펴봐.",
                    "바뀐 글자는 딱 하나뿐이야 — 아주 작은 차이가 큰 문제를 만들었어.",
                },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "바뀐 설계도대로 단백질이 새로 만들어져. 사슬이 접히면서 입체 모양이 되는 걸 지켜보자.",
                    "다 접히면 색깔 지도가 켜질 거야. 이 색은 AI가 이 부분 모양을 얼마나 자신 있게 예측했는지 보여줘.",
                    "특히 이 근처가 많이 흔들리네. 여기가 우리가 눈여겨봐야 할 자리야.",
                },
                hints = new[]
                {
                    "정상 단백질과 이상 단백질을 나란히 두고 12번 자리만 비교해봐.",
                    "색이 노랗거나 빨간 곳은 AI도 확신이 없다는 뜻이니 조심해서 살펴보자.",
                },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "단백질 표면을 지나 안쪽 빈 공간으로 들어가 볼게. 이 자리가 바로 열쇠 구멍이야.",
                    "12번 자리에 새로 생긴 노란 원자가 보이지? 저게 우리가 노리는 특별한 자리야.",
                    "이 원자는 다른 것과 아주 잘 달라붙어. 여기에 딱 맞는 열쇠를 꽂으면 두 번 다시 안 풀리는 결합을 만들 수 있어.",
                },
                hints = new[]
                {
                    "이 열쇠 구멍은 정상 단백질에는 거의 없어. 모양이 바뀐 단백질에서만 새로 열려.",
                    "12번 자리를 찾아봐. 노란빛 원자가 열쇠 구멍 안쪽을 향하고 있어.",
                },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "후보물질 네 개를 준비했어. 하나씩 열쇠 구멍에 가져다 대 보자.",
                    "구멍에 들어가는 것만으로는 부족해. 그 원자를 실제로 붙잡는 '팔'이 달려 있어야 해.",
                },
                hints = new[]
                {
                    "모양만 맞는 것과 진짜로 붙잡는 것은 달라. 잘 살펴봐.",
                    "달라붙는 특수한 '팔(반응기)'이 있는지 확인해봐.",
                    "숫자가 마이너스(-)로 클수록 더 세게 붙는다는 뜻이야. 플러스(+)면 오히려 밀려나.",
                },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "원자 속 세계에서 나와서 다시 연구실로 돌아왔어.",
                    "화면에 결과가 떴어! 이상 신호를 내던 세포는 줄어들고, 정상 세포는 그대로 잘 지내고 있어.",
                    "정상 단백질은 건드리지 않았으니 부작용도 적을 거야.",
                },
                hints = new[]
                {
                    "혹시 결과 화면에서 '내성'이라는 말이 보이면, 세포가 또 다른 방법으로 저항할 수도 있다는 뜻이야.",
                },
    ...
    private static void FillEgfrL858R(QuestDefinition quest)
    {
        ...
        quest.stages = new[]
        {
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "EGFR은 세포 표면에 있는 신호 안테나야. 원래는 신호가 와야만 켜지는데, 지금은 아니야.",
                    "858번 자리를 살펴보자. 여기 모양이 조금 달라졌어.",
                    "작은 부분 하나가 바뀌면서 안테나가 신호도 없이 계속 켜진 채로 고정돼 버렸어.",
                },
                hints = new[] { "858번 자리를 클릭해봐. 빨갛게 반짝이고 있어." },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "색이 파란색일수록 AI가 이 모양을 자신 있게 예측한 거야. 노란빛 부분은 조심해서 보자.",
                },
                hints = new[] { "리본 모양에서 원자 단위까지 들어가면 하나하나 자세히 볼 수 있어." },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "원래 이 자리엔 세포를 움직이게 하는 연료 같은 물질이 들어가. 그 자리를 찾으면 그게 표적이야.",
                },
                hints = new[] { "790번 자리 근처를 살펴보자. 나중에 안테나가 '방어'에 쓰는 자리이기도 해." },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[] { "처음 만든 열쇠는 세포가 방법을 바꾸는 바람에 더 안 듣게 됐어. 새로운 열쇠를 찾아보자." },
                hints = new[] { "797번 자리에 단단히 달라붙는 열쇠를 찾아보자." },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[] { "정상 안테나까지 막으면 피부 같은 곳에서 부작용이 생길 수 있어. 결과를 확인해보자." },
                hints = new[] { "변형된 것만 골라서 막을수록 정상 세포는 더 안전해." },
    ...
    private static void FillAbl1T315I(QuestDefinition quest)
    {
        ...
        quest.stages = new[]
        {
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "다시 만나서 반가워! 이번엔 예전에 우리가 해결했던 사건이 돌아왔어.",
                    "만성골수성백혈병이라는 병은 BCR-ABL이라는 융합 단백질이 계속 '자라라!' 신호를 보내서 생겨. " +
                    "그때 Imatinib이라는 열쇠로 이 신호를 멈췄었지.",
                    "그 열쇠는 ABL1 단백질의 315번 자리 근처, ATP가 들어가는 좁은 문 앞에서 딱 맞물렸었어.",
                },
                hints = new[] { "315번 자리를 기억해둬. 이번 사건의 진짜 무대야." },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "⏳ 18개월 후... 화면이 잠깐 어두워졌다가, 다시 같은 자리를 비춰줄게.",
                    "315번 자리를 다시 봐봐. 작았던 Thr(트레오닌)가 훨씬 커다란 Ile(아이소류신)로 바뀌었어.",
                    "저 빨갛게 빛나는 곳이 바로 그 자리야. 곁사슬이 커지면서 예전 열쇠가 들어가던 틈을 막아버렸어.",
                },
                hints = new[] { "315번 자리를 클릭해봐. 빨갛게 반짝이며 커진 게 보일 거야." },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "이 포켓은 예전과 똑같은 자리이지만, 315번 문지기 때문에 입구 모양이 완전히 달라졌어.",
                    "이제 필요한 건 이 문지기를 정면으로 밀어붙이는 열쇠가 아니라, 살짝 돌아가는 열쇠야.",
                },
                hints = new[] { "포켓 안쪽 다른 잔기들(248, 271, 317, 318번 등)은 그대로야 — 오직 315번만 달라졌어." },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "후보물질을 다섯 개 준비했어. Imatinib, Nilotinib, Dasatinib은 전에도 쓰였던 익숙한 " +
                    "열쇠들이고, 나머지 둘은 이번 사건을 위해 새로 나온 후보야.",
                    "하나씩 포켓에 가져다 대 보자. 커진 315번 자리를 피해가는 모양인지 잘 살펴봐.",
                },
                hints = new[]
                {
                    "결합력(친화도) 숫자가 마이너스로 클수록 더 세게 붙는다는 뜻이야.",
                    "곧게 뻗은 좁은 연결고리를 가진 열쇠가 좁아진 틈을 통과하기 유리해.",
                },
            ...
            new QuestStageBriefing
            {
                ...
                assistantLines = new[]
                {
                    "돌아보면 Imatinib, Nilotinib, Dasatinib 모두 315번 자리에서 막혔었지.",
                    "하지만 Ponatinib은 곧게 뻗은 연결고리 하나로 그 벽을 피해갔어. 키나아제 활동이 완전히 멈췄어!",
                    "약물의 아주 작은 구조적 변화가 내성을 이겨내느냐 마느냐를 가르는 거야. " +
                    "(A small structural change in a drug can determine whether resistance is overcome.)",
                },
                hints = new[]
                {
                    "궁금하면 물어봐 — Asciminib이라는 약은 아예 다른 자리(미리스토일 포켓)를 노려서 이 " +
                    "문지기를 신경 쓸 필요가 없어.",
                },
