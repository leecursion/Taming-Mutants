...
export async function handleDocking(request,env) {
  if(!env.DOCKING_SERVICE_URL) return json({error:'도킹 계산 서버가 연결되지 않았어요. DOCKING_SERVICE_URL 설정이 필요합니다.'},503);
  ...
  try {
    const upstream=await fetch(new URL('/api/docking',env.DOCKING_SERVICE_URL),{
      method:'POST',signal:AbortSignal.timeout(55000),headers:{'Content-Type':'application/json','X-App-Token':env.DOCKING_SERVICE_TOKEN||''},body:JSON.stringify(body)
    });
